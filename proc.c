// MAMAN 02
// This file contains the process table and the process structure.
// Keeps track of each running process including PID, PPID, size and state.
// T.Z: 329604441
// Name: Sacha HADJADJ
...
//T.Z: 329604441
//NAME: Sacha HADJADJ
//Implementation of cps 141 for containers
...
cps141(void) 
{
  ...
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
    ...
    struct pid_ns *root = myproc()->nsproxy->pid_ns;
    
    while(root && root->parent)
      root = root->parent;
      
    local_pid = 0;
    if(p->nsproxy && p->nsproxy->pid_ns)
      local_pid = get_pid_for_ns(p, p->nsproxy->pid_ns);
      
    ext_pid = get_pid_for_ns(p, root);
    
    ppid = 0;
    if(p->parent)
      ppid = get_pid_for_ns(p->parent, root);
